...
- `customPoliciesPaths` - o primeiro arquivo que a define prevalece
...
`src/hooks/builtin-policies.ts` define todas as 40 políticas embutidas como objetos `BuiltinPolicyDefinition`:
...
1. Lê `customPoliciesPaths` da configuração; ignora se ausente.
...
│   ├── builtin-policies.ts       # 40 definições de políticas
