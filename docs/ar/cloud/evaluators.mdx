...
1. **اكتب مُسجّل.** أنشئ خدمة HTTP صغيرة تقرأ نسخة من جلسة وترجع نقاط. تشحن FailproofAI Cloud مرجعاً يعمل يمكنك نسخه. انظر [كتابة مُقيّم مع SDK](/ar/cloud/evaluators).
...
المُقيّم هو **خدمتك** — لا تشحن FailproofAI Cloud مُقيّماً افتراضياً، لذا تبني وتشغل أينما تشغل خدماتك. يعمل تحت أي خادم ASGI (على سبيل المثال `uvicorn my_evaluator:app`؛ خدم المسارات `/health` و `/config` و `/evaluate` من [عقد HTTP](/ar/cloud/evaluators)، ثم وجّه الخادم إليه (انظر [تكوين الخادم](/ar/cloud/evaluators)).
...
- **`/dashboards`**: عروض صحة تقييم محفوظة (انظر [القوائس](/ar/cloud/evaluators) أدناه).
