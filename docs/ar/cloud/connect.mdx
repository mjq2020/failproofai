...
[Creating scoped keys →](/ar/cloud/access)
...
<Tip>
  ...
  fleet view, which is the one gap the [fleet page](/ar/cloud/fleet) is built to make obvious.
</Tip>
...
<Accordion title="This key lacks the policies:pull permission (403)">
  ...
  the existing key. See [Access](/ar/cloud/access).
</Accordion>
...
<Accordion title="Every tool call is being denied">
  That is the [fail-closed guarantee](/ar/daemon#fail-closed) doing its job: on a configured
...
<Card title="Access and permissions" icon="key" href="/ar/cloud/access">
  Creating a key with exactly the two permissions this needs.
</Card>

<Card title="The daemon" icon="gear" href="/ar/daemon">
