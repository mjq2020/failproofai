...
1. **Scrivi uno scorer.** Crea un piccolo servizio HTTP che legge una trascrizione della sessione e restituisce i punteggi. FailproofAI Cloud fornisce un riferimento funzionante che puoi copiare. Vedi [Scrivere un valutatore con l'SDK](/it/cloud/evaluators).
...
Il valutatore è **il tuo servizio** — FailproofAI Cloud non fornisce un valutatore predefinito, quindi lo crei e lo esegui dove esegui i tuoi servizi. Viene eseguito sotto qualsiasi server ASGI (ad esempio `uvicorn my_evaluator:app`); servi le rotte `/health`, `/config` e `/evaluate` dal [contratto HTTP](/it/cloud/evaluators), poi punta il server su di esso (vedi [Configurare il server](/it/cloud/evaluators)).
...
- **`/dashboards`**: viste di salute eval salvate (vedi [Dashboard](/it/cloud/evaluators) sotto).
