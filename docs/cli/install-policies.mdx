...
| Flag | Description |
|------|-------------|
| `--cli <name>` | Agent CLI(s) to install for — any of the 12 supported CLIs (see [agent support](/agent-support)); space-separated (e.g. `--cli claude codex copilot`) or repeated. Omit to detect installed CLIs and prompt. |
...
| `--scope local` | Claude only — installs into `<cwd>/.claude/settings.local.json`. No other CLI has a `local` scope. |
...
When `--custom <path>` is provided, the file is validated immediately - it must call `customPolicies.add()` at least once. The resolved path is saved to `policies-config.json` as `customPoliciesPaths`.
