...
[Creating scoped keys →](/tr/cloud/access)
...
<Tip>
  ...
  fleet view, which is the one gap the [fleet page](/tr/cloud/fleet) is built to make obvious.
</Tip>
...
<Accordion title="This key lacks the policies:pull permission (403)">
  ...
  the existing key. See [Access](/tr/cloud/access).
</Accordion>
...
<Accordion title="Every tool call is being denied">
  That is the [fail-closed guarantee](/tr/daemon#fail-closed) doing its job: on a configured
...
<Card title="Access and permissions" icon="key" href="/tr/cloud/access">
  Creating a key with exactly the two permissions this needs.
</Card>

<Card title="The daemon" icon="gear" href="/tr/daemon">
