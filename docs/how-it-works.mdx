...
silently disables every policy is not a guarded machine. A protocol-version mismatch
...
prompt, so the agent goes back and finishes the job. Where failproofai does not subscribe to a turn-end event
...
After each event, one line per evaluated decision — allows included — is appended to
...
Every evaluated event is logged, including allows — an allow row is what distinguishes
"a policy ran and permitted this" from "no policy covered this event". This log is
