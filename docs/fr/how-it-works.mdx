...
`src/hooks/builtin-policies.ts` définit les 39 politiques intégrées sous forme d'objets `BuiltinPolicyDefinition` :
...
1. Lecture de `customPoliciesPath` depuis la configuration ; ignoré s'il est absent.
