...
1. **כתוב מדרג.** הקם שירות HTTP קטן שקורא תמליל של סשן ומחזיר ציונים. FailproofAI Cloud משלח התייחסות עובדת שאתה יכול להעתיק. ראה [כתיבת מעריך עם ה-SDK](/he/cloud/evaluators).
...
ההערכה היא **השירות שלך** — FailproofAI Cloud לא משלח מעריך ברירת מחדל, כך שאתה בונה והרץ אותו במקום שבו אתה מריץ את השירותים שלך. הוא פועל תחת כל שרת ASGI (לדוגמה `uvicorn my_evaluator:app`); הגיש את נתיבי `/health`, `/config` ו-`/evaluate` מ-[חוזה HTTP](/he/cloud/evaluators), ואז הצביע את השרת אליו (ראה [הגדרת השרת](/he/cloud/evaluators)).
...
- **`/dashboards`**: צפיות בריאות eval שמורה (ראה [לוחות](/he/cloud/evaluators) להלן).
