...
description: "All 40 built-in policies that catch common agent failure modes"
icon: shield
---

FailproofAI ships with 40 built-in policies that catch common agent failure modes. Each fires on a specific hook event and tool. Nineteen accept parameters, so you can tune them without writing code. Five workflow policies enforce a commit → push → PR → CI pipeline before the agent is allowed to finish.
...
<Note>
  Not sure which to turn on? `failproofai config` offers **Recommended** — 15 policies
...
<Warning>
  The five **Workflow** policies need a turn-end gate, and on two CLIs failproofai does not subscribe to one —
...
<Tip>
Every policy supports an optional `hint` field in `policyParams`. The hint is appended to the deny or instruct message Claude sees, giving actionable guidance without modifying policy code. Works with built-in, custom, and convention policies. See [Configuration → hint](/configuration#hint-—-accepted-by-every-policy) for details.
</Tip>
...
Detect credentials in tool output. Sanitizer policies fire on **PostToolUse** — after the tool has run. When a secret pattern matches, the policy raises a decision naming what was found.

What that decision does depends on the CLI. On Codex and Copilot it replaces the tool result the model reads. On the other ten, `PostToolUse` is observation-only: the model still reads the original output, with a note appended that a secret was detected. These policies do not edit the output in place, and the tool has already run either way — so treat them as detection, not prevention. To stop a secret being read at all, use the `PreToolUse` policies `block-env-files`, `protect-env-vars` and `block-secrets-write`.

### `sanitize-jwt`

**Event:** PostToolUse (all tools)  
**Default:** Detects JWT tokens (three base64url segments separated by `.`).
...
**Event:** PostToolUse (all tools)  
**Default:** Detects common API key formats: Anthropic (`sk-ant-`), OpenAI (`sk-`), GitHub PATs (`ghp_`), AWS access keys (`AKIA`), Stripe keys (`sk_live_`, `sk_test_`), and Google API keys (`AIza`).
...
**Event:** PostToolUse (all tools)  
**Default:** Detects database connection strings that contain embedded credentials (e.g. `postgresql://user:password@host/db`).
...
**Event:** PostToolUse (all tools)  
**Default:** Detects PEM blocks (`-----BEGIN PRIVATE KEY-----`, `-----BEGIN RSA PRIVATE KEY-----`, etc.).
...
**Event:** PostToolUse (all tools)  
**Default:** Detects `Authorization: Bearer <token>` headers where the token is 20 or more characters.
...
No policy-specific parameters. Use the cross-cutting [`hint`](/configuration#hint-—-accepted-by-every-policy) to suggest alternatives:
