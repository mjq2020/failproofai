...
Nếu bạn chạy **daemon failproofaid** (xem [`failproofai config`](/vi/cli/config)),
...
Bật nó trong `~/.failproofai/config.json`:
...
- Tiến độ được ghi vào `~/.failproofai/state/audit-schedule.json` (lần chạy cuối cùng, tiếp theo đến hạn). Daemon sở hữu tệp đó — thay đổi tốc độ trong `config.json`.
