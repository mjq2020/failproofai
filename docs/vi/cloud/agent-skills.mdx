...
|---|---|---|
| **`agenteye-cli`** | Read your data and run your organization — *"which sessions errored today?"*, *"give CI a key that can only push events"* | Drives the [CLI](/vi/cloud/cli) as you |
...
reach for after [`failproofai audit`](/vi/audit) shows you what your agents actually do.
...
**Prerequisites:** the [`agenteye` CLI](/vi/cloud/cli) installed and on `PATH`, your dashboard
...
<Warning>
  **This skill has your full permissions, including writes.** It runs the CLI *as you*, so
  ...
  This is a different blast radius from the [in-dashboard assistant](/vi/cloud/assistant),
...
The [SDK](/vi/cloud/sdk) is small — thirteen event methods, all keyword-only — and a coding
...
The hard part of evaluation is not the code. The [HTTP contract](/vi/cloud/evaluators) is
...
<Card title="Acting on an audit" icon="list-check">
  Run [`failproofai audit`](/vi/audit), and it sorts the findings into what a policy can
...
agent CLI ([`failproofai config`](/vi/cli/config)). No cloud account, no login.
...
| | Agent skills | [In-dashboard assistant](/vi/cloud/assistant) |
|---|---|---|
...
<Card title="CLI reference" icon="terminal" href="/vi/cloud/cli">
  Every command, flag, and JSON shape the CLI skill drives.
</Card>

<Card title="CLI recipes" icon="code" href="/vi/cloud/cli-recipes">
...
<Card title="Evaluators" icon="ruler" href="/vi/cloud/evaluators">
...
<Card title="Custom policies" icon="pen-to-square" href="/vi/custom-policies">
