...
|---|---|---|
| **`agenteye-cli`** | Read your data and run your organization — *"which sessions errored today?"*, *"give CI a key that can only push events"* | Drives the [CLI](/de/cloud/cli) as you |
...
reach for after [`failproofai audit`](/de/audit) shows you what your agents actually do.
...
**Prerequisites:** the [`agenteye` CLI](/de/cloud/cli) installed and on `PATH`, your dashboard
...
<Warning>
  **This skill has your full permissions, including writes.** It runs the CLI *as you*, so
  ...
  This is a different blast radius from the [in-dashboard assistant](/de/cloud/assistant),
...
The [SDK](/de/cloud/sdk) is small — thirteen event methods, all keyword-only — and a coding
...
The hard part of evaluation is not the code. The [HTTP contract](/de/cloud/evaluators) is
...
<Card title="Acting on an audit" icon="list-check">
  Run [`failproofai audit`](/de/audit), and it sorts the findings into what a policy can
...
agent CLI ([`failproofai config`](/de/cli/config)). No cloud account, no login.
...
| | Agent skills | [In-dashboard assistant](/de/cloud/assistant) |
|---|---|---|
...
<Card title="CLI reference" icon="terminal" href="/de/cloud/cli">
  Every command, flag, and JSON shape the CLI skill drives.
</Card>

<Card title="CLI recipes" icon="code" href="/de/cloud/cli-recipes">
...
<Card title="Evaluators" icon="ruler" href="/de/cloud/evaluators">
...
<Card title="Custom policies" icon="pen-to-square" href="/de/custom-policies">
