...
Für jedes Transkript wird jedes Tool-Use-Ereignis durch die 39 integrierten Policies **und** durch 8 audit-exklusive Detektoren wiedergegeben, die Muster erkennen, die noch nicht durch Laufzeit-Policies abgedeckt sind. Die Zählungen werden pro Policy/Detektor über alle Sitzungen hinweg aggregiert.
...
- Der Fortschritt wird in `~/.failproofai/state/audit-schedule.json` gespeichert (letzter Durchlauf,
  nächste Fälligkeit). Der Daemon besitzt diese Datei — ändere den Rhythmus in `config.toml`.
