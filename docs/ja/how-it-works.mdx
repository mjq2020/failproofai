...
- `customPoliciesPaths` - 最初に定義したファイルが優先
- `llm` - 最初に定義したファイルが優先
...
`src/hooks/builtin-policies.ts` は 40 個の組み込みポリシーを `BuiltinPolicyDefinition` オブジェクトとして定義しています。
...
1. 設定から `customPoliciesPaths` を読み取り、存在しない場合はスキップします。
...
│   ├── builtin-policies.ts       # 40 個のポリシー定義
