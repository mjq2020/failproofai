...
|---|---|---|
| **`agenteye-cli`** | Read your data and run your organization — *"which sessions errored today?"*, *"give CI a key that can only push events"* | Drives the [CLI](/ja/cloud/cli) as you |
...
reach for after [`failproofai audit`](/ja/audit) shows you what your agents actually do.
...
**Prerequisites:** the [`agenteye` CLI](/ja/cloud/cli) installed and on `PATH`, your dashboard
...
<Warning>
  **This skill has your full permissions, including writes.** It runs the CLI *as you*, so
  ...
  This is a different blast radius from the [in-dashboard assistant](/ja/cloud/assistant),
...
The [SDK](/ja/cloud/sdk) is small — thirteen event methods, all keyword-only — and a coding
...
The hard part of evaluation is not the code. The [HTTP contract](/ja/cloud/evaluators) is
...
<Card title="Acting on an audit" icon="list-check">
  Run [`failproofai audit`](/ja/audit), and it sorts the findings into what a policy can
...
agent CLI ([`failproofai config`](/ja/cli/config)). No cloud account, no login.
...
| | Agent skills | [In-dashboard assistant](/ja/cloud/assistant) |
|---|---|---|
...
<Card title="CLI reference" icon="terminal" href="/ja/cloud/cli">
  Every command, flag, and JSON shape the CLI skill drives.
</Card>

<Card title="CLI recipes" icon="code" href="/ja/cloud/cli-recipes">
...
<Card title="Evaluators" icon="ruler" href="/ja/cloud/evaluators">
...
<Card title="Custom policies" icon="pen-to-square" href="/ja/custom-policies">
